...
TODO:
1. Объспечить возможность инициализыции дерева при создании;
2. Добавить в дерево следующие методы:
	??? Clear();		//Удаляет все элементы дерева.
	??? Erase(???);		//Удаляет из дерева заданный элемент.
	??? Depth(???);		//Возвращает гдубину дерева.			DONE
	??? TreePrint(???);	//Выводит дерево как дерево.
	??? Balance(???);	//Балансирует Бинарное Дерево (вырожденоне дерево приводит к идеально сбалансированному)
3. *** Реализовать Красно-Черное Дерево;

DONE:
1. Добавить в дерево следующие методы:
	??? MinValue(???);	//Возвращает минимальное значение в дереве	DONE
	??? MaxValue(???);	//Возвращает минимальное значение в дереве	DONE
	??? Count(???);		//Возвращает количество элементов в дереве	DONE
	??? Sum(???);		//Возвращает сумму дерева					DONE
	??? Avg(???);		//Среднее-арифметическое сумму дерева		DONE
2. Обеспечить вызов методов дерева из Main() без необходимости передач в них корня;	DONE
